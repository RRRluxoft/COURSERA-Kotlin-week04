...
class SquareBoardImpl(override val width: Int) : SquareBoard {
    ...
    val cells: List<Cell> = (0 until width * width)
            .map {
    ...
    override fun getCell(i: Int, j: Int): Cell = getCellOrNull(i, j) ?:
        throw IllegalArgumentException("Both arguments (i, j) should be between 1 and $width. getCell was called with ($i, $j)")
...
private constructor(private val squareBoard: SquareBoard) : GameBoard<T>, SquareBoard by squareBoard {
    ...
    override fun filter(predicate: (T?) -> Boolean): Collection<Cell> = cellValues.filterValues {predicate(it)}
            .map { it.key }
